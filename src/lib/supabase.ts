...
export const SupabaseService = {
  ...
  async saveHistoryEvent(event: HistoryEvent): Promise<void> {
    if (isPlaceholder) return;
    const { error } = await supabase.from('history_events').upsert(event);
    if (error) throw error;
  },
  ...
  async saveMapEvent(event: MapEvent): Promise<void> {
    if (isPlaceholder) return;
    const { error } = await supabase.from('map_events').upsert(event);
    if (error) throw error;
  },
  async deleteMapEvent(id: string): Promise<void> {
    if (isPlaceholder) return;
  ...
  // Meter (Votes)
  async getMeterStats(): Promise<Record<string, number>> {
    if (isPlaceholder) return { '😊': 0, '😐': 0, '😟': 0 };
    const { data, error } = await supabase.from('akwaba_votes').select('vote');
    const stats: Record<string, number> = { '😊': 0, '😐': 0, '😟': 0 };
    if (error) return stats;
    data?.forEach((v: any) => { 
      if (stats[v.vote] !== undefined) stats[v.vote]++; 
    });
    return stats;
  },
  async submitMeterVote(userId: string, vote: string): Promise<void> {
    if (isPlaceholder) return;
    const { error } = await supabase.from('akwaba_votes').upsert({ 
      user_id: userId, 
      vote, 
  ...
  // Diaspora
  async getDiasporaStories(onlyApproved = false): Promise<DiasporaStory[]> {
    if (isPlaceholder) return [];
    let query = supabase.from('diaspora_stories').select('*').order('created_at', { ascending: false });
    if (onlyApproved) query = query.eq('approved', true);
    const { data, error } = await query;
    if (error) return [];
    return (data as DiasporaStory[]) || [];
  },
  async saveDiasporaStory(story: DiasporaStory): Promise<void> {
    if (isPlaceholder) return;
    const { error } = await supabase.from('diaspora_stories').upsert(story);
    if (error) throw error;
  },
  async approveStory(id: string): Promise<void> {
    if (isPlaceholder) return;
    const { error } = await supabase.from('diaspora_stories').update({ approved: true }).eq('id', id);
    if (error) throw error;
  },
  async deleteStory(id: string): Promise<void> {
    if (isPlaceholder) return;
    const { error } = await supabase.from('diaspora_stories').delete().eq('id', id);
    if (error) throw error;
  },
  ...
  async saveSuccessStory(story: SuccessStory): Promise<void> {
    if (isPlaceholder) return;
    const { error } = await supabase.from('success_stories').upsert(story);
    if (error) throw error;
  },
  ...
  async getQuizzes(): Promise<Quiz[]> {
    if (isPlaceholder) return [];
    const { data, error } = await supabase.from('quizzes').select('*, quiz_questions(*)').order('date', { ascending: false });
    if (error) return [];
    return (data as Quiz[]) || [];
  },
  async saveQuiz(quiz: any): Promise<void> {
    ...
    const { data: savedQuiz, error: quizError } = await supabase.from('quizzes').upsert(quizData).select().single();
    if (quizError) throw quizError;
    
    if (questions && questions.length > 0) {
      await supabase.from('quiz_questions').delete().eq('quiz_id', quizData.id);
      const questionsWithId = questions.map((q: any) => ({ ...q, quiz_id: quizData.id }));
      const { error: qsError } = await supabase.from('quiz_questions').insert(questionsWithId);
